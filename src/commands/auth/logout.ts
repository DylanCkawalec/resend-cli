...
export const logoutCommand = new Command('logout')
  .description('Remove the saved Resend API key from the local credentials file')
  ...
  .action(async (_opts, cmd) => {
    ...
    if (!existsSync(configPath)) {
      if (!globalOpts.json && isInteractive()) {
        ...
        outputResult({ success: true, already_logged_out: true }, { json: globalOpts.json });
      }
      return;
    }
    ...
    try {
      ...
      outputError(
        { message: errorMessage(err, 'Failed to remove credentials file'), code: 'remove_failed' },
    ...
    if (!globalOpts.json && isInteractive()) {
      ...
      outputResult({ success: true, config_path: configPath }, { json: globalOpts.json });
    }
  });
